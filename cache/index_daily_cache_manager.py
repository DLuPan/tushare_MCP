...
class IndexDailyCacheManager:
    """A股指数日线行情数据专用缓存管理器
    
    特点：
    1. 使用专门的表存储A股指数日线行情数据，字段直接映射到数据库列
    2. 缓存策略：永不失效（数据永久保存）
    ...
    def _init_database(self):
        ...
        # 创建A股指数日线行情数据表
        # 根据 Tushare index_daily 接口的数据格式设计
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS index_daily_data (
                ts_code TEXT NOT NULL,           -- TS指数代码（如：000300.SH）
        ...
        # 创建索引以提升查询性能
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_index_daily_ts_code ON index_daily_data(ts_code)
        ''')
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_index_daily_trade_date ON index_daily_data(trade_date)
        ''')
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_index_daily_ts_code_date ON index_daily_data(ts_code, trade_date)
        ''')
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_index_daily_created_at ON index_daily_data(created_at)
        ''')
    ...
    def save_index_daily_data(self, df: pd.DataFrame) -> int:
        ...
        参数:
            df: 包含指数日线行情数据的DataFrame，必须包含 ts_code 和 trade_date 列
        ...
        for _, row in df.iterrows():
            try:
                ...
                # 记录错误但继续处理其他记录
                print(f"保存A股指数日线行情数据时出错: {str(e)}", file=__import__('sys').stderr)
                continue
    ...
    def get_index_daily_data(
        ...
        参数:
            ts_code: 指数代码（如：000300.SH，支持多个指数，逗号分隔）
    ...
    def has_data(self, ts_code: str, trade_date: Optional[str] = None) -> bool:
        ...
        if trade_date:
            ...
            cursor.execute(f'''
                SELECT COUNT(*) FROM index_daily_data
    ...
    def clear_index_daily_data(self, ts_code: Optional[str] = None) -> int:
        ...
        if ts_code:
            cursor.execute('DELETE FROM index_daily_data WHERE ts_code = ?', (ts_code,))
        else:
            cursor.execute('DELETE FROM index_daily_data')
...
# 创建全局A股指数日线行情缓存管理器实例
index_daily_cache_manager = IndexDailyCacheManager()
