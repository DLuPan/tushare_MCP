...
class StockWeeklyCacheManager:
    """股票周线行情数据专用缓存管理器
    
    特点：
    1. 使用专门的表存储股票周线行情数据，字段直接映射到数据库列
    2. 缓存策略：永不失效（数据永久保存）
    3. 主键：(ts_code, trade_date)，确保同一股票的同一周只有一条数据
    4. 支持按股票代码、交易日期范围查询
    5. 支持多股票查询（逗号分隔的股票代码）
    """
    ...
    def _init_database(self):
        ...
        # 创建索引以提升查询性能
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_stock_weekly_ts_code ON stock_weekly_data(ts_code)
        ''')
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_stock_weekly_trade_date ON stock_weekly_data(trade_date)
        ''')
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_stock_weekly_ts_code_date ON stock_weekly_data(ts_code, trade_date)
        ''')
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_stock_weekly_created_at ON stock_weekly_data(created_at)
        ''')
    ...
    def save_stock_weekly_data(self, df: pd.DataFrame) -> int:
        ...
        参数:
            df: 包含股票周线行情数据的DataFrame，必须包含 ts_code 和 trade_date 列
        ...
        for _, row in df.iterrows():
            try:
                ...
                # 记录错误但继续处理其他记录
                print(f"保存股票周线行情数据时出错: {str(e)}", file=__import__('sys').stderr)
                continue
    ...
    def clear_stock_weekly_data(self, ts_code: Optional[str] = None) -> int:
        ...
        if ts_code:
            cursor.execute('DELETE FROM stock_weekly_data WHERE ts_code = ?', (ts_code,))
        else:
            cursor.execute('DELETE FROM stock_weekly_data')
...
# 创建全局股票周线行情缓存管理器实例
stock_weekly_cache_manager = StockWeeklyCacheManager()
